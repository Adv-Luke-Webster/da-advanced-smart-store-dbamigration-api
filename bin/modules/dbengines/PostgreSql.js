...
number idleTimeoutMillis - lifespan for unused connections
    ...
    let qBits = instance.jdbcURL.split('?')
    const location = qBits[0].substr(18) // = "host[:port]/database"
    const aBits = location.split('/')
    const hostBits = aBits[0].split(':')
    connConfig.host = hostBits[0]
    if (hostBits.length > 1) { connConfig.port = hostBits[1] }
    if (aBits.length > 1) { connConfig.database = aBits[1] }
...
function executeFetchList (context, params) {
  ...
  try {
    const bpar = _postgreParams(params)
  ...
  try {
    ...
    tableOrColumnExists(context, tableName, columns[0], noPrefix).then(function (texists) {
      if (texists.data.tableExists && texists.data.columnExists) {
        ...
        const sSQL = 'select ix.indisunique as "is_unique",' +
  ...
  try {
    db.tx(function (t) {
      ...
      _.each(sts, function (st, iX) {
        const bpar = _postgreParams(st.params)
...
function resolveSQL (sqlDef) {
  return dbEngineLib.resolveSQL(sqlDef, 'postgresql', 'qqq').then(function (st) {
    const rx = /\@([A-Za-z0-9\$\#_]+)/g // Find SQL identifier words starting with @
    if (_.isArray(st)) {
