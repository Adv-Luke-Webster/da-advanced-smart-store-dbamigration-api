...
function tableOrColumnExists(context, tableName, columnName, noPrefix) {
    ...
    try
    {
        ...
        var owner = config.get('DatabaseInstances')[0].jdbcUser;
    ...
    try
    {
        ...
        tableOrColumnExists(context, tableName, columns[0], noPrefix).then(function(texists) {
            ...
            if (texists.data.tableExists && texists.data.columnExists) {
                ...
                var sSQL = "select i.INDEX_NAME as \"index_name\","
...
function _oracleParams(params) {
    ...
    _.each(params, function(param, key) {
        var value = _oracleCleanValue(param.value, param.type);
        op[key] = {
...
function _oracleParamTypes(mssqlType) {
    var retType = oracledb.STRING; // (2001) Bind as JavaScript String type.  Can be used for most database types.
    switch (mssqlType) {
        case sql.Bit:
...
function simpleExec(context, st) {
    ...
    try
    {
        var bpar = _oracleParams(st.params);
        var bopts = {};
...
function resolveSQL(sqlDef) {
    return dbEngineLib.resolveSQL(sqlDef, "oracle", "QQQ").then(function(st){  
        var rx = /\@([A-Za-z0-9\$\#_]+)/g, // Find SQL identifier words starting with @ so we can change to oracle ":param" syntax
            rxF = /\bFALSE\b/ig,    // Our statments use the ANSI standard TRUE and FALSE constants for booleans. 
